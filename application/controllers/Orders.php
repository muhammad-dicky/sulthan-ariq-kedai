	...
	public function fetchOrdersData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if($value['paid_status'] == 1) {
				$paid_status = '<span class="label label-success">Paid Cash</span>';	
			} else if($value['paid_status'] == 3) {
				$paid_status = '<span class="label label-success">Paid Qris</span>';
			}
			else {
	...
	*/
	public function printDiv($id)
	{
		...
		if($id) {
			...
			// $paid_status = ($order_data['paid_status'] == 1) ? "Paid Cash" : "Unpaid";

			if ($order_data['paid_status'] == 1) {
				$paid_status = "Paid Cash";
			} else if ($order_data['paid_status'] == 3) {
				$paid_status = "Paid Qris";
			}
			else {
				$paid_status = "Unpaid";
			}
